...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    const circulo1 = circle(3);
    expect(typeof circulo1).toBe('object');
    expect(Object.keys(circulo1)).toHaveLength(3);
    expect(circle()).toBeUndefined();
    const circulo2 = circle(2);
    expect(circulo2.circumference).toBe(12.56);
    const circulo3 = circle(3);
    expect(circulo3.area).toBe(28.259999999999998);
    // Teste se a função retorna, em um objeto, os dados corretos de um círculo de raio 3.
    expect(circulo3.circumference).toBe(18.84);
  });
});
